...
- La Etapa 10B.1 prepara un ambiente `alpha` fail-closed con Cloudflare Workers Free condicionado al smoke real de CPU, una única configuración Hyperdrive con query caching deshabilitado, y Supabase Free para PostgreSQL, Auth y Storage. No existe aún despliegue ni recurso aprovisionado; el procesamiento multimedia Alpha sigue bloqueado hasta conectar un proceso Node persistente. La guía operativa vigente es `docs/runbook-alpha.md`.
