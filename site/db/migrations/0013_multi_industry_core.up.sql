...
ALTER TABLE public.templates
  ADD CONSTRAINT templates_industry_valid CHECK (
    industry_key IN ('restaurant', 'gym')
  );

ALTER TABLE public.sites
  ADD COLUMN industry_key text NOT NULL DEFAULT 'restaurant',
  ADD CONSTRAINT sites_industry_valid CHECK (
    industry_key IN ('restaurant', 'gym')
  );
...
BEGIN
  SELECT tenant_id,industry_key INTO expected_tenant,site_industry
  ...
  END IF;
  SELECT version.status,version.content_schema_key,template.industry_key,
         version.minimum_schema_version,version.maximum_schema_version
    INTO version_status,version_schema,template_industry,
  ...
  IF version_status IS NULL
    ...
    OR template_industry <> site_industry
...
CREATE FUNCTION app_private.enforce_site_industry_consistency()
RETURNS trigger LANGUAGE plpgsql SET search_path=pg_catalog AS $function$
BEGIN
...
END
$function$;
CREATE TRIGGER sites_industry_consistency
...
BEGIN
  ...
  SELECT EXISTS(
    ...
    JOIN public.templates template ON template.id=version.template_id
    JOIN public.sites site ON site.id=NEW.site_id
    WHERE version.id=NEW.template_version_id
      AND version.status='active' AND template.status='active'
      AND template.industry_key=site.industry_key
  ...
  USING (
    ...
    AND EXISTS(
      ...
      WHERE site.tenant_id=app_context.current_tenant_id()
        AND site.industry_key=templates.industry_key
...
AS $function$
  WITH candidate AS (
    SELECT site.id,site.slug,site.status AS site_status,
      site.industry_key,site.current_publication_id,site.tenant_id,
  ...
  SELECT candidate.id,candidate.slug,
    CASE
      WHEN candidate.tenant_status='active'
       ...
       AND template.industry_key=candidate.industry_key
      THEN 'published'
      WHEN candidate.tenant_status='active'
    ...
    CASE WHEN publication.id IS NOT NULL
      AND template.industry_key=candidate.industry_key THEN version.renderer_key END,
    CASE WHEN publication.id IS NOT NULL
      AND template.industry_key=candidate.industry_key THEN publication.schema_key END,
    CASE WHEN publication.id IS NOT NULL
      AND template.industry_key=candidate.industry_key THEN publication.schema_version END,
    CASE WHEN publication.id IS NOT NULL
      AND template.industry_key=candidate.industry_key THEN publication.id END,
    CASE WHEN publication.id IS NOT NULL
      AND template.industry_key=candidate.industry_key THEN publication.publication_number END,
    CASE
      WHEN candidate.tenant_status='active'
       AND candidate.site_status='active'
       AND version.status IN ('active','deprecated')
  ...
  LEFT JOIN public.templates template ON template.id=version.template_id
  LEFT JOIN LATERAL (
