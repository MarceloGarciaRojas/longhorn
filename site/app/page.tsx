...
const faqs = [
  ["¿Qué es Longhorn?", "Longhorn es una propuesta SaaS B2B para ayudar a pymes a ordenar su presencia y operación digital desde una experiencia simple."],
...
function Brand({ inverse = false }: { inverse?: boolean }) {
  return <span className={`brand ${inverse ? "brand-inverse" : ""}`}><i aria-hidden="true">＋</i><b>Longhorn</b></span>;
}
  ...
  return <div className="landing">
    ...
    <main>
      ...
      <section className="section intro">
        ...
        <p className="section-lead">Longhorn parte de problemas concretos: información dispersa, tareas repetidas y herramientas difíciles de mantener.</p>
      ...
      <section className="section solution" id="solucion">
        ...
        <div className="ecosystem-card"><div className="card-title"><b>Ecosistema Longhorn</b><span>ALCANCE MVP</span></div>{modules.map(([n, title, copy]) => <div className="module-row" key={n}><i>{n}</i><span><b>{title}</b><small>{copy}</small></span></div>)}</div>
      </section>
      ...
      <section className="section news" id="noticias"><div><span className="kicker">Noticias Longhorn</span><h2>Avances del proyecto.</h2></div><div className="news-grid"><article><span>Diseño · Julio 2026</span><h3>Nueva experiencia visual en validación</h3><p>Estamos probando una interfaz más simple, intuitiva y consistente para el rubro piloto.</p><a href="#inicio">Leer actualización <b>→</b></a></article><article><span>Producto · Próximamente</span><h3>Definición de las tres plantillas iniciales</h3><p>El siguiente hito documental establecerá el contenido y alcance de cada alternativa.</p><a href="#planes">Conocer los planes <b>→</b></a></article></div></section>

      <section className="cta"><span className="kicker">Construyamos claridad</span><h2>Conoce cómo podría verse Longhorn en tu pyme.</h2><button className="pill light" onClick={() => go("onboarding")}>Ver experiencia demo <span>→</span></button></section>
    </main>
    <footer><div><Brand inverse /><p>Gestión digital simple para pymes que quieren crecer con control.</p></div><div><b>Prototipo</b><button onClick={() => go("dashboard")}>Panel demostrativo</button><button onClick={() => go("design")}>Guía visual</button></div><small>© 2026 Longhorn · Etapa de preparación documental</small></footer>
  </div>;
}
...
function Login({ go }: { go: (view: View) => void }) {
  ...
  return <><Landing go={go} /><ModalShell eyebrow="Acceso de demostración" title="Bienvenido a Longhorn" close={() => go("landing")}><p className="modal-copy">Usa cualquier correo y contraseña para recorrer este prototipo. No se envían ni almacenan datos.</p><form onSubmit={submit} className="form"><label>Correo<input required type="email" defaultValue="demo@longhorn.cl" /></label><label>Contraseña<input required type="password" defaultValue="demolonghorn" /></label><button className="pill primary" type="submit">Ingresar al panel <span>→</span></button></form><button className="text-button" onClick={() => go("onboarding")}>¿Primera vez? Explora el onboarding</button></ModalShell></>;
}
...
function VideoDemo({ go }: { go: (view: View) => void }) {
  return <ModalShell eyebrow="Video demostrativo" title="Así funciona Longhorn" close={() => go("landing")} wide>
    ...
    <div className="video-placeholder" role="img" aria-label="Animación provisional del proceso de Longhorn">
